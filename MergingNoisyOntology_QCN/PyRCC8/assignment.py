...
def staticUnassignedVars(Matrix):
   ...
   if globs['varheuristic'] == 0: # none
      ...
      if globs['split'] == 0: # splitting based on set of base relations
         stL.extend(sorted([(0,(i,j)) for i in range(len(Matrix)) for j in range(i+1,len(Matrix))]))
      ...
      if globs['split'] == 0: # splitting based on set of base relations
         ...
         if globs['scope'] == 0: # local scope
            stL.extend(sorted([(16*len(bsplit[Matrix[i][j]-1][1]) + ew[Matrix[i][j]-1],(i,j)) for i in range(len(Matrix)) for j in range(i+1,len(Matrix))]))
         elif globs['scope'] == 1: #global scope
            stL.extend(sorted([(neighborCons(Matrix, i, j) + ew[Matrix[i][j]-1],(i,j)) for i in range(len(Matrix)) for j in range(i+1,len(Matrix))]))

      elif globs['split'] == 1: # splitting based on horn set
         # check for scope of constrainedness
         if globs['scope'] == 0: # local scope
            stL.extend(sorted([(16*len(hsplit[Matrix[i][j]-1][1]) + ew[Matrix[i][j]-1],(i,j)) for i in range(len(Matrix)) for j in range(i+1,len(Matrix))]))
         elif globs['scope'] == 1: #global scope           
            stL.extend(sorted([(neighborCons(Matrix, i, j) + ew[Matrix[i][j]-1],(i,j)) for i in range(len(Matrix)) for j in range(i+1,len(Matrix))]))
      ...
      # check for splitting to be used
      if globs['split'] == 0: # splitting based on set of base relations
         stL.extend([(0,(i,j)) for i in range(len(Matrix)) for j in range(i+1,len(Matrix))])
         shuffle(stL)
...
def findAllUnAssignVarCH(Matrix):
   ...
   if globs['split'] == 0: # splitting based on set of base relations
      ...
      if globs['scope'] == 0: # local
         l = [(16*len(bsplit[Matrix[i][j]-1][1]) + ew[Matrix[i][j]-1],(i,j)) for i in range(len(Matrix)) for j in range(i+1,len(Matrix)) if bsplit[Matrix[i][j]-1][0] > 1]
      if globs['scope'] == 1: # global
         l = [(neighborCons(Matrix, i, j) + ew[Matrix[i][j]-1],(i,j)) for i in range(len(Matrix)) for j in range(i+1,len(Matrix)) if bsplit[Matrix[i][j]-1][0] > 1]
   
   elif globs['split'] == 1: # splitting based on horn set
      # check for scope of constrainedness
      if globs['scope'] == 0: # local
         l = [(16*len(hsplit[Matrix[i][j]-1][1]) + ew[Matrix[i][j]-1],(i,j)) for i in range(len(Matrix)) for j in range(i+1,len(Matrix)) if hsplit[Matrix[i][j]-1][0] > 1]
      if globs['scope'] == 1: # global
         l = [(neighborCons(Matrix, i, j) + ew[Matrix[i][j]-1],(i,j)) for i in range(len(Matrix)) for j in range(i+1,len(Matrix)) if hsplit[Matrix[i][j]-1][0] > 1]
...
def findAllUnAssignVarR(Matrix):
   ...
   # check for splitting to be used
   if globs['split'] == 0: # splitting based on set of base relations
      l = [(i,j) for i in range(len(Matrix)) for j in range(i+1,len(Matrix)) if bsplit[Matrix[i][j]-1][0] > 1] 
   elif globs['split'] == 1: # splitting based on horn set
      l = [(i,j) for i in range(len(Matrix)) for j in range(i+1,len(Matrix)) if hsplit[Matrix[i][j]-1][0] > 1] 
