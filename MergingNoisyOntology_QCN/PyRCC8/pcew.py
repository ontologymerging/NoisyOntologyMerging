...
def PCew(ConMatrix, m = -1, n = -1):
   ...
   if m == -1 and n == -1:
      ...
      map(add_task,[(i,j) for i in range(Vars) for j in range(i+1,Vars) if ConMatrix[i][j] != DALL], [(ew[ConMatrix[i][j]-1]) for i in range(Vars) for j in range(i+1,Vars) if ConMatrix[i][j] != DALL])
   else:
